   ...
   /**
    ...
    protected function findColumns($table)
    {
        ...
        if ($this->db->isISeries) {
            $sql = <<<SQL
                SELECT lcase(c.column_name) AS name,
                ...
                WHERE qsys2.syscst.constraint_type = 'PRIMARY KEY'
                  ...
                  on lcase(x.column) = lcase(c.column_name)
                WHERE UPPER(c.table_name) = UPPER(:table1)
    ...
    protected function findConstraints($table)
    {
        if ($this->db->isISeries) {
            ...
            SELECT
              ...
              lcase(parent.column_name) AS pk,
              lcase(child.column_name) AS fk
...
SQL;
            ...
            SELECT
                ...
                lcase(fk.colname) AS fk,
                lcase(pk.colname) AS pk
            FROM
    ...
    public function findUniqueIndexes($table)
    {
        ...
        if ($this->db->isISeries) {
            $sql = <<<SQL
                ...
                lcase(qsys2.syskeycst.column_name) As column
                FROM qsys2.syscst
                INNER JOIN qsys2.syskeycst
                  ON qsys2.syscst.constraint_name = qsys2.syskeycst.constraint_name
                 AND qsys2.syscst.table_schema = qsys2.syskeycst.table_schema
        ...
        if ($this->db->isISeries) {
            $sql = <<<SQL
                ...
                WHERE TABLE_TYPE IN ('P','T','V')
                  AND SYSTEM_TABLE = 'N'
                  AND lcase(TABLE_SCHEMA) = lcase(:schema)
                ORDER BY TABLE_NAME
SQL;
        }else {
...
SQL;
            ...
            if ($schema !== '') {
                $sql .= ' AND lcase(t.tabschema) = lcase(:schema)';
            }
        }
        $command = $this->db->createCommand($sql);
    ...
    protected function loadTablePrimaryKey($tableName) {
        ...
        $sql = <<<SQL
            SELECT CASE
                     ...
                     ELSE i.indname
                   END AS name,
                   lcase(t.colname) AS column_name
    ...
    protected function loadTableUniques($tableName) {
        ...
        $sql = <<<SQL
            SELECT i.indname AS name,
                   lcase(ic.colname) AS column_name
    ...
    protected function loadTableChecks($tableName) {
        ...
        $sql = <<<SQL
            SELECT c.constname AS name,
                   lcase(cc.colname) AS column_name,
...
SQL;
    ...
    protected function loadTableDefaultValues($tableName) {
        ...
        $sql = <<<SQL
            SELECT lcase(c.colname) AS column_name,
    ...
    protected function loadTableForeignKeys($tableName) {
        ...
        $sql = <<<SQL
            SELECT ref.constname AS name,
                   lcase(fk.colname) AS column_name,
                   ref.reftabschema AS ref_schema,
                   ref.reftabname AS ref_table,
                   lcase(pk.colname) AS ref_column,
    ...
    public function getLastInsertID($sequenceName = '')
    {
        if ($this->db->isActive) {
            ...
            return $this->db->useMaster(function (Connection $db) use ($sequenceName) {
                return $db->createCommand("SELECT IDENTITY_VAL_LOCAL() FROM SYSIBM.SYSDUMMY1")->queryScalar();
            });
        } else {
